  ...
  <div class="app-container">
    <div class="wrap">
      <div class="header">
        <h3>菜单列表</h3>
        <div @click="apiMask">
          <img src="@/assets/role_images/add.png" alt="" /><span>新建根级菜单</span>
        </div>
      </div>
      <div class="content">
        <!-- table 表头  -->
        ...
        <div class="base-table" style="margin-top: 20px">
          ...
          >
            <template slot="icon" slot-scope="prop">
              <!-- {{prop.row.children}} -->
              <icon :icon="prop.row.icon" color="" />
            </template>
            <template slot="keepalive" slot-scope="{ row }">
              <el-switch
                v-model="row.keepalive"
                @change="changeMenu(row)"
              >
              </el-switch>
            </template>
            <template slot="is_hidden" slot-scope="{ row }">
              <el-switch
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { label: "图标", prop: "icon", width: "100px", slot: true },
        { label: "排序", prop: "order", width: "50px" },
        { label: "访问路径", prop: "path", width: "120px" },
        { label: "跳转路径", prop: "redirect", width: "120px" },
        { label: "组件路径", prop: "component", width: "150px" },
        { label: "保活", prop: "keepalive", width: "100px", slot: true },
        { label: "隐藏", prop: "is_hidden", width: "100px", slot: true },
        { label: "创建日期", prop: "created_at", width: "100px" },
      ],
      //编辑
      roleItem: {}, //每一行表格的值
      //搜索
      //标题参数
      formFields: [{ prop: "path", label: "路径" }],
      formFieldsData: {},
      api_path: "",
      api_summary: "",
      api_tags: "",
  ...
  methods: {
    changeMenu(e) {
      console.log(e, e.keepalive);
      this.$api('system').updateMenu(e).then(res => {
        this.$message[res.code == 200 ? 'success' : 'error'](res.msg||'未知错误');
      })
    },
    getApiList() {
      console.log("requestList", this.pageInfo);
      this.$api("system")
        .menuList(this.pageInfo.page, this.pageInfo.page_size)
        .then((res) => {
          if (res && res.code && res.code == 200) {
            this.listData = (res.data||[]).map(e => {
              // e.hasChildren = !!(e.children && e.children.length);

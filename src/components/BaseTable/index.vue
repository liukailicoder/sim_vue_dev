 ...
 * @version:
 * @Author: 曹盼盼
 * @Date: 2024-05-22 14:45:45
 * @LastEditors: 曹盼盼
 * @LastEditTime: 2024-05-27 18:06:02
-->
<!--
 * @Descripttion:
 * @version:
 * @Author: 曹盼盼
 * @Date: 2024-05-59 14:45:45
 * @LastEditors: 刘凯丽----新增序号列判断（column.slot）
-->
<template>
  <div class="table">
    <el-table :data="tableData" style="width: 100%; min-height: 400px">
      <el-table-column v-for="column in dataList" :key="column.prop" :prop="column.prop" :label="column.label" :width="column.width">
        <template slot-scope="scope">
          <div v-if="column.slot">
            <slot :name="column.prop" :row="scope.row" :index="scope.$index" :$index="scope.$index"></slot>
          </div>
          <template v-else>
